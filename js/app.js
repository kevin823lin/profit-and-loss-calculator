...
const 取得無條件進位 = (數值) => Math.ceil(數值);
const 取得無條件捨去 = (數值) => Math.floor(數值);
...
class 計算器 {
  ...
  精算原始手續費(手續費) {
    return 取得無條件捨去(手續費 < this.最低手續費 ? this.最低手續費 : 手續費);
  }

  精算折讓手續費(手續費) {
    return 取得無條件進位(手續費 < this.最低手續費 ? this.最低手續費 : 手續費);
  }
  ...
  處理報價() {
    ...
    const 報價 = this.偏移列表
      .map((初始偏移) => {
        ...
        const 原始買入手續費 = this.精算原始手續費(成本 * 公定手續費費率);
        const 原始賣出手續費 = this.精算原始手續費(市值 * 公定手續費費率);
        const 買入手續費 = this.精算折讓手續費(原始買入手續費 * this.手續費折扣);
        const 賣出手續費 = this.精算折讓手續費(原始賣出手續費 * this.手續費折扣);
        ...
        const 原始買入手續費字樣 = 取得無條件進位(原始買入手續費 * 百) / 百 < this.最低手續費 ? `(${this.格式化(原始買入手續費)})` : "";
        const 原始賣出手續費字樣 = 取得無條件進位(原始賣出手續費 * 百) / 百 < this.最低手續費 ? `(${this.格式化(原始賣出手續費)})` : "";
